...
import { Upload, CheckCircle2, Loader2, FileText, Users, Shield, ExternalLink } from "lucide-react";
...
const UniversityAdmin = () => {
  const [certs, setCerts] = useState<MockCertificate[]>(getAllCertificates());
  const [bulkIssuing, setBulkIssuing] = useState(false);
  const [progress, setProgress] = useState(0);

  useEffect(() => subscribe(() => setCerts([...getAllCertificates()])), []);

  const totalIssued = certs.filter((c) => c.status === "Anchored").length;
  const totalStudents = new Set(certs.map((c) => c.studentId)).size;
  ...
  const handleBulkIssue = () => {
    ...
    const newCert: MockCertificate = {
      certificateId: `PV-2026-${String(Math.floor(Math.random() * 99999)).padStart(5, "0")}`,
    ...
    const interval = setInterval(() => {
      ...
      if (pct >= 100) {
        ...
        updateCertStatus(newCert.certificateId, "Anchored");
        setBulkIssuing(false);
        toast({ title: "Certificate Anchored!", description: `${newCert.recipientName} — ${newCert.certificateId}` });
      }
    }, 50);
  };
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] font-mono uppercase tracking-wider text-muted-foreground border-b border-border/40">
                ...
                <th className="pb-3 pr-4 hidden lg:table-cell">Student ID</th>
            ...
            <tbody>
              {certs.map((cert, i) => (
                <motion.tr key={cert.certificateId} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                  ...
                  <td className="py-3.5 pr-4 hidden md:table-cell">
                    <span className="font-mono text-xs text-highlight">{cert.certificateId}</span>
                  </td>
                  <td className="py-3.5 pr-4 hidden lg:table-cell">
                    <span className="font-mono text-xs text-muted-foreground">{cert.studentId}</span>
                  </td>
                  <td className="py-3.5 pr-4 hidden sm:table-cell text-xs text-muted-foreground">{cert.issueDate}</td>
                  <td className="py-3.5 pr-4">
